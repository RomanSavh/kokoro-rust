...
use crate::{onnx::ort_wrapper::OrtWrapper, utils::format_debug_prefix};
...
type Output = (ArrayBase<OwnedRepr<f32>, IxDyn>, Option<Vec<f32>>);
type PrepareOutput = Vec<(Cow<'static, str>, SessionInputValue<'static>)>;
...
impl OrtWrapper for OrtKoko {
    fn set_session(&mut self, sess: Session) {
...
impl OrtKoko {
    ...
    fn prepare_inputs(
        ...
        styles: Vec<Vec<f32>>,
        speed: f32,
    ) -> Result<PrepareOutput, Box<dyn std::error::Error>> {
        ...
        match strategy {
            ...
            ModelStrategy::Timestamped(sess) => {
                ...
                let durations_vec = outputs[DURATIONS]
                    ...
                    .map(|(_, d)| d.to_vec())
                    .map_err(|_| format!(
                        "Timestamped Model Error: Expected output tensor '{DURATIONS}' of type f32. \
